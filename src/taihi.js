...
(async () => {
  while (true) {
    ...
    if (select === "1") {
      while (true) {
        ...
        if (files.length > 0) {
          ...
          if (
            ...
            tokens = fs
              .readFileSync(files[Number(file) - 1], "utf8")
              .trim()
              .split(/\r\n|\n/);
            console.log(
      ...
      if (tokens) {
        output_folder = `./Results/${new Date()
          .toLocaleString()
          .replace(/\//g, "-")
          .replace(/:/g, ".")}`;
        ...
        await Promise.all(
          tokens.map((token) => {
            return limit(() => check(token)).then(() => {
              counter.checked++;
              process.title = `Discord Token Checker - Coded by @L2 | Checked: ${counter.checked.toLocaleString()} (${(
          ...
          console.log(
            `\nResults: \n{success: ${chalk.green(
              counter.success
            )}, require: ${chalk.yellow(counter.require)}, invalid: ${chalk.red(
              counter.invalid
            )}}`
          );
